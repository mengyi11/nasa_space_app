...
const HF_ML_ENDPOINT = "https://huggingface.co/spaces/deviix-x/NASASPACEPOTATO/api/predict"; // Hugging Face ML Endpoint
...
export async function GET(req) {
  try {
    ...
    // Fetch Real-Time Air Quality Data prediction from Hugging face
    ...
    const forecast = await getForecastFromHF(aqi);
...
//   city: "Singapore",
//   country: "SG",
//   aqi: Math.floor(Math.random() * 300), // 0-299随机数
...
//   const aqi = Math.floor(Math.random() * 300); // 每次请求刷新随机值
//   const data = {
//     ...fakeAQIData,
//     aqi,
//     pm25: (Math.random() * 150).toFixed(1),
...
//6. Fetch forecast from hugging face

async function getForecastFromHF(currentAQI) {
  try {
    const response = await axios.post(HF_ML_ENDPOINT, { currentAQI });
    // The ML space returns forecast in response.data.data
    return response.data.data || [];
  } catch (err) {
    console.warn("HF ML forecast failed, using fallback:", err.message);
    // Fallback: simple trend
    return [currentAQI, currentAQI+5, currentAQI+10, currentAQI+5, currentAQI, currentAQI-5, currentAQI-10].map(x => Math.max(0, x));
  }
}
